...
st.set_page_config(page_title="Vestidor Virtual AI", page_icon="👕")

# Título e Interfaz limpia
st.title("👕 Tu Vestidor Virtual Inteligente")
st.markdown("---")

# Barra lateral para configuración
with st.sidebar:
    st.header("Configuración")
    api_key = st.text_input("Ingresá tu API Key de Gemini:", type="password")
...
if st.button("Ver cómo me queda 😎"):
    if not url_producto or not foto_usuario or not api_key:
        st.error("¡Ups! Asegurate de completar todos los campos.")
    else:
        try:
            client = genai.Client(api_key=api_key)
            
            with st.spinner("✨ Analizando el estilo de la prenda..."):
                # Lógica interna de scraping y IA
                ...
                # Identificación de nombre real del objeto
                h1 = soup.find("h1")
                titulo = h1.text.strip().lower() if h1 else "prenda"
                prompt_filtro = f"De este título: '{titulo}', extrae solo el nombre del objeto. Una palabra."
                ...
                instruccion = f"Sustituye la ropa del usuario por la {nombre_obj}. Mantén color y marca exactos."
            ...
            st.error(f"Hubo un error técnico. Verificá el link o la API Key.")

st.info("Nota: Esta es una representación por IA con fines ilustrativos.")
